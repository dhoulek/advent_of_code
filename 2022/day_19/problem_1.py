...
def play_minute(robots, resources, step):    
    ...
    if step > 1:        
        ...
        max_upgrades = [int(min(resources / costs[i])) for i in range(4)]
        # max_upgrades = [1]*4
        # upgrades = []
        # print('m', maximum_geodes, step, robots, resources, max_upgrades)
        ...
        # increased by one
        max_optimistic = no_more_robots + step*(step-1)//2#robots[0]*step/(costs[2,0]*costs[3,0])
        # print(max_optimistic)
        # print('max', max_optimistic)
        if (max_optimistic <= maximum_geodes):
            # no need to check anything - we cannot beat the current maximum
            # print('no chance')
            return
        # play_minute(robots, resources+robots, step-1)
        # upgrades = [(ore, clay, obs, geo) for geo in reversed(range(max_upgrades[3]+1))\
                    # for obs in reversed(range(max_upgrades[2]+1))\
                    # for clay in range(max_upgrades[1]+1)\
                    # for ore in range(max_upgrades[0]+1)]
        upgrades = product(*[reversed(range(max_upgrades[i]+1)) for i in range(4)])        
        for upgrade in upgrades:
            cost = np.dot(costs.T, upgrade)
            # if resources[1] > 13:# and upgrade[2] > 0:
        # upgrades = product(*[range(max_upgrades[i]+1) for i in range(4)])
                # print(upgrade, cost, resources, robots)
            ...
            #     # if resources[1]>10:
                # print('buy', upgrade, cost, resources, robots, sum(cost<=resources)==4)                 
            if sum(cost <= resources) == 4:
                play_minute(robots+upgrade, resources-cost+robots, step-1)
        ...
        # if resources[1] > 13:
        # if resources[2]+robots[2] >= 2:
            # print('.', resources+robots, robots)
...
# print(play_minute(robots, [24, 65, 3, 0], costs, 2))
play_minute(robots, resources, 20)
