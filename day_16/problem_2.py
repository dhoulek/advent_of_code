...
def make_move(time, path, time_ele, path_ele, to_visit):
    global max_relief
    global best_path
    global best_time
    global best_path_ele
    global best_time_ele
    current = path[-1]
    now = time[-1]
    current_ele = path_ele[-1]
    now_ele = time_ele[-1]
    if len(to_visit) > 0 and ((now < 30) or (now_ele < 30)):
        if now <= now_ele:
            # moving me
            for n in to_visit:
                ...
                # print(duration)
                make_move(time+[min(duration+now, 30)], path+[n], time_ele, path_ele, to_visit-{n})
        else:
            # moving elephant
            for n in to_visit:
                # length + 1 for opening
                duration = dist[all_valves.index(current_ele), all_valves.index(n)] + 1
                # print(duration)
                make_move(time, path, time_ele+[min(duration+now_ele, 30)], path_ele+[n], to_visit-{n})
    else:
        # print(path, time)
        relief = eval_path(time, path)+eval_path(time_ele, path_ele)
        if relief > max_relief:
            print(max_relief)
            max_relief = relief
            best_path = path
            best_time = time
            best_path_ele = path_ele
            best_time_ele = time_ele
        
make_move(time, path, time_ele, path_ele, to_visit)
print(f'maximum relief is {max_relief}')
print(f'this is obtained for valves:')
print(f'  me:      {best_path}')
print(f'   at:     {best_time}')
print(f' elephant: {best_path_ele}')
print(f'   at:     {best_time_ele}')